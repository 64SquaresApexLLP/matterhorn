...
const Dashboard = () => {
  ...
  const recentClients = [
    { id: 1, name: "TechCorp Solutions", type: "Corporate Client", status: "Paid", avatar: "TC", color: "#3B82F6", amount: "$15,500" },
    { id: 2, name: "Johnson & Associates", type: "Law Firm", status: "Pending", avatar: "JA", color: "#10B981", amount: "$8,750" },
    { id: 3, name: "Global Industries Ltd", type: "Corporate Client", status: "Paid", avatar: "GI", color: "#F59E0B", amount: "$22,300" },
    { id: 4, name: "Smith Family Trust", type: "Private Client", status: "Overdue", avatar: "SF", color: "#EF4444", amount: "$5,200" },
    { id: 5, name: "Innovation Partners", type: "Startup", status: "Paid", avatar: "IP", color: "#8B5CF6", amount: "$12,400" }
  ];

  const recentInvoices = [
    { id: 1, invoice: "INV-2025-001", client: "TechCorp Solutions", status: "Paid", amount: "$15,500", date: "2025-06-20" },
    { id: 2, invoice: "INV-2025-002", client: "Johnson & Associates", status: "Pending", amount: "$8,750", date: "2025-06-18" },
    { id: 3, invoice: "INV-2025-003", client: "Global Industries", status: "Paid", amount: "$22,300", date: "2025-06-15" },
    { id: 4, invoice: "INV-2025-004", client: "Smith Family Trust", status: "Overdue", amount: "$5,200", date: "2025-06-12" }
  ];
  ...
  const users = ["All Users", "John Smith", "Sarah Johnson", "Michael Brown", "Emily Davis", "David Wilson"];
  ...
  const totalMatters = currentUserData.reduce((sum, item) => sum + item.count, 0);

  const getStatusColor = (status) => {
    switch (status) {
      case "Draft": return "#ef4444";
      case "Pending Manager": return "#f59e0b";
      case "Invoiced": return "#3b82f6";
      case "Completed by Manager": return "#10b981";
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className={`glass-card p-6 rounded-2xl card-hover ${animationTrigger ? "animate-scale-in" : ""}`} style={{ animationDelay: "0.1s" }}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-slate-800">Top Revenue Generators</h3>
          ...
          <div className={`glass-card px-6 py-6 rounded-2xl card-hover ${animationTrigger ? "animate-scale-in" : ""}`} style={{ animationDelay: "0.2s" }}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-gray-800">Revenue Statistics</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 glass-card p-6 rounded-2xl card-hover ${animationTrigger ? "animate-slide-up" : ""}`}>
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-xl font-semibold text-slate-800">Monthly Revenue</h3>
                <p className="text-slate-600 text-sm">Previous Month - November 2024</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-slate-600">November</p>
                <p className="text-2xl font-semibold text-slate-800">$456,000</p>
              </div>
            </div>
            <div className="h-100">
...
>
      ...
      <div key={index} className="flex items-center space-x-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className={`lg:col-span-2 glass-card p-6 rounded-2xl card-hover ${animationTrigger ? "animate-slide-up" : ""}`} style={{ animationDelay: "0.4s" }}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-slate-800">Invoice Status Breakdown</h3>
              <span className="text-blue-500 text-sm">Overall Progress</span>
            </div>
            <div className="h-70">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={invoiceStatusData} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
                  <XAxis 
                    dataKey="status" 
                    axisLine={false} 
          ...
          {/* Summary Stats */}
          <div className={`glass-card p-6 rounded-2xl card-hover ${animationTrigger ? "animate-scale-in" : ""}`} style={{ animationDelay: "0.5s" }}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-semibold text-green-600">$456,000</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600">Pending Amount</span>
                <span className="text-lg font-semibold text-orange-600">$125,000</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600">Overdue Amount</span>
                <span className="text-lg font-semibold text-red-600">$23,500</span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className={`glass-card p-6 rounded-2xl card-hover ${animationTrigger ? "animate-slide-up" : ""}`} style={{ animationDelay: "0.5s" }}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-slate-800">Recent Client Billing</h3>
                ...
                <div key={client.id} className="flex items-center justify-between p-3 hover:bg-white/60 rounded-lg transition-all">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-medium text-green-600">{client.amount}</p>
                    </div>
                  </div>
          ...
          <div className={`glass-card p-6 rounded-2xl card-hover ${animationTrigger ? "animate-slide-up" : ""}`} style={{ animationDelay: "0.6s" }}>
            ...
            <div className="space-y-3">
              {recentInvoices.map((invoice) => (
                <div key={invoice.id} className="p-3 hover:bg-white/60 rounded-lg transition-all">
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="font-medium text-slate-800 text-sm">{invoice.invoice}</h4>
                    <span className="font-semibold text-green-600 text-sm">{invoice.amount}</span>
                  </div>
                  <p className="text-xs text-slate-500 mb-1">{invoice.client}</p>
                  <div className="flex justify-between items-center">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
        ...
        <div className={`glass-card p-6 rounded-2xl card-hover ${animationTrigger ? "animate-slide-up" : ""}`} style={{ animationDelay: "0.7s" }}>
  <div className="flex justify-between items-center mb-6">
    <h3 className="text-xl font-semibold text-slate-800">Overall Status Breakdown</h3>
  ...
  <div className="space-y-4">
    ...
    <div className="space-y-2">
      ...
      <div className="w-full bg-gray-200 rounded-full h-6">
        <div className="bg-red-500 h-6 rounded-full flex items-center justify-start pl-3" style={{ width: '12%' }}>
    ...
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        <span className="text-sm font-medium text-slate-700">Pending Manager</span>
        <span className="text-sm text-slate-600">8</span>
      </div>
      <div className="w-full bg-gray-200 rounded-full h-6">
        <div className="bg-orange-500 h-6 rounded-full flex items-center justify-start pl-3" style={{ width: '21%' }}>
    ...
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        <span className="text-sm font-medium text-slate-700">Invoiced</span>
        <span className="text-sm text-slate-600">34</span>
      </div>
      <div className="w-full bg-gray-200 rounded-full h-6">
        <div className="bg-blue-500 h-6 rounded-full flex items-center justify-start pl-3" style={{ width: '34%' }}>
    ...
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        <span className="text-sm font-medium text-slate-700">Completed by Manager</span>
        <span className="text-sm text-slate-600">45</span>
      </div>
      <div className="w-full bg-gray-200 rounded-full h-6">
        <div className="bg-green-500 h-6 rounded-full flex items-center justify-start pl-3" style={{ width: '45%' }}>
  ...
  <div className="mt-6 pt-4 border-t border-slate-200">
    <div className="flex justify-between items-center">
      <span className="text-sm font-medium text-slate-700">Total Matters</span>
      <span className="text-lg font-semibold text-slate-800">99</span>
    </div>
  </div>
</div>
